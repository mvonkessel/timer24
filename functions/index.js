...
exports.idleCheck = functions
  ...
  .onRun(async () => {
    ...
    if (status === 'running') {
      ...
      } else if (d.timerEnd && d.timerEnd > Date.now()) {
        // Timer active: silent status update (wakes SW + updates persistent notification)
        const remMin = Math.max(1, Math.round((d.timerEnd - Date.now()) / 60000));
        await sendPush(token,
          '\u23F1 ' + (d.task || 'Timer'),
          remMin + ' min \u00FCbrig', 'pomo-status', []);
      }
      return;
    }
    ...
    if (status === 'break') {
      if (d.breakEnd && Date.now() > d.breakEnd) {
        ...
        // Active break: send silent push to wake SW for IDB break timer check
    ...
    if (d.isShaming && d.idleSince) {
      ...
      await sendPush(token,
        ...
        'idle-nag',
        [500, 60, 500, 60, 500, 60, 800]);
      // Also update persistent status notification (silent)
      await sendPush(token,
        '\u{1F480} Schandzeit +' + idleMin + 'm',
        'Jetzt starten!', 'pomo-status', []);
      return;
    }
