...
self.addEventListener('notificationclick', e => {
  ...
  // All other notifications: focus existing window or open new one
  e.waitUntil(
    self.clients.matchAll({ type: 'window' }).then(clients => {
      if (clients.length > 0) {
        clients[0].focus();
...
self.addEventListener('push', e => {
  ...
  e.waitUntil(
    self.registration.showNotification(title, {
      ...
      checkStoredTimer();
      checkStoredBreak();
      // If page is still alive (throttled but not killed): wake it to trigger loud alarm
      self.clients.matchAll({ type: 'window' }).then(clients => {
        clients.forEach(c => c.postMessage({ type: 'FCM_WAKE', title, body, tag }));
      });
    })
  );
});
