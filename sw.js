...
self.addEventListener('activate', e => {
  e.waitUntil(
    ...
    ).then(() => { checkStoredTimer(); checkStoredNags(); checkStoredBreak(); checkStoredStatus(); })
  );
  self.clients.claim();
});
...
self.addEventListener('fetch', e => {
  ...
  checkStoredTimer();
  // NOTE: checkStoredNags, checkStoredBreak, checkStoredStatus intentionally NOT here.
  // They fire notifications — harmful on page load (foreground). They run on push + activate only.
...
self.addEventListener('message', e => {
  ...
  if (data.type === 'START_TIMER') {
    ...
    clearTimeout(_breakTimeout);_breakTimeout=null;idbClearBreak();
    alarmActive = false;
    _timerGen++;
  ...
  // ── Persistent Status Notification (self-refreshing) ──
  if (data.type === 'UPDATE_STATUS') {
    // data: { title, body, mode?, endTime?, idleSince?, task? }
    const statusData = {
      mode: data.mode || 'static', // 'timer','break','idle','static'
      endTime: data.endTime || null,
      idleSince: data.idleSince || null,
      task: data.task || '',
      staticTitle: data.title || '',
      staticBody: data.body || ''
    };
    idbSaveStatus(statusData).then(() => {
      refreshStatusNotification(statusData);
      startStatusRefreshLoop();
    });
  }
  if (data.type === 'CLEAR_STATUS') {
    stopStatusRefreshLoop();
    idbClearStatus();
    self.registration.getNotifications({ tag: 'pomo-status' }).then(n => n.forEach(x => x.close()));
  }
});
  ...
  try {
    ...
    if (Date.now() >= data.breakEnd) {
      if (!data.notified) {
        ...
        // Update status notification to show idle
        await self.registration.showNotification('\uD83D\uDC80 Schandzeit', {
          body: 'Pause vorbei \u2014 jetzt starten!',
          icon: './icon-192.png',
          badge: './icon-192.png',
          tag: 'pomo-status',
          renotify: false,
...
// ── IDB Status Helpers (for self-refreshing notification) ──
async function idbSaveStatus(data) {
  try { const db = await idbOpen(); const tx = db.transaction(IDB_STORE, 'readwrite'); tx.objectStore(IDB_STORE).put(data, 'status'); db.close(); } catch (e) {}
}
async function idbLoadStatus() {
  try { const db = await idbOpen(); return new Promise(r => { const tx = db.transaction(IDB_STORE, 'readonly'); const req = tx.objectStore(IDB_STORE).get('status'); req.onsuccess = () => { db.close(); r(req.result || null); }; req.onerror = () => { db.close(); r(null); }; }); } catch (e) { return null; }
}
async function idbClearStatus() {
  try { const db = await idbOpen(); const tx = db.transaction(IDB_STORE, 'readwrite'); tx.objectStore(IDB_STORE).delete('status'); db.close(); } catch (e) {}
}

function refreshStatusNotification(s) {
  if (!s) return;
  let title, body;
  if (s.mode === 'timer' && s.endTime) {
    const rem = Math.max(0, Math.ceil((s.endTime - Date.now()) / 1000));
    if (rem > 0) {
      const m = Math.floor(rem / 60), sec = rem % 60;
      title = '\u23F1 ' + (s.task || 'Timer');
      body = m + ':' + String(sec).padStart(2, '0') + ' \u00FCbrig';
    } else {
      title = '\uD83D\uDD14 ' + (s.task || 'Timer') + ' fertig!';
      body = 'App \u00F6ffnen';
    }
  } else if (s.mode === 'break' && s.endTime) {
    const rem = Math.max(0, Math.ceil((s.endTime - Date.now()) / 1000));
    if (rem > 0) {
      const m = Math.floor(rem / 60), sec = rem % 60;
      title = '\u2615 Pause';
      ...
      const over = Math.floor((Date.now() - s.endTime) / 60000);
      title = '\uD83D\uDC80 Schandzeit';
      body = 'Pause vorbei! +' + over + 'm';
    }
  } else if (s.mode === 'idle' && s.idleSince) {
    const el = Math.floor((Date.now() - s.idleSince) / 60000);
    title = '\uD83D\uDC80 Schandzeit +' + el + 'm';
    body = 'Jetzt starten!';
  } else {
    title = s.staticTitle || '';
    body = s.staticBody || '';
  ...
  self.registration.showNotification(title, {
    ...
    tag: 'pomo-status', renotify: false, requireInteraction: true, silent: true
  }).catch(() => {});
}
...
function startStatusRefreshLoop() {
  ...
  _statusRefreshInterval = setInterval(async () => {
    try {
      const s = await idbLoadStatus();
      if (!s) { stopStatusRefreshLoop(); return; }
      refreshStatusNotification(s);
    } catch (e) {}
  }, 10000);
}
function stopStatusRefreshLoop() {
  clearInterval(_statusRefreshInterval);
  _statusRefreshInterval = null;
}

// On any SW wake-up: restart status refresh if status data exists
async function checkStoredStatus() {
  try {
    const s = await idbLoadStatus();
    if (s) { refreshStatusNotification(s); startStatusRefreshLoop(); }
  } catch (e) {}
}
...
self.addEventListener('notificationclick', e => {
  ...
  if (tag === 'nudge' || tag === 'push-cascade' || tag === 'fcm-push' || tag === 'rabbit-hole' || tag === 'morning-penalty' || tag === 'pomo-status') {
    e.waitUntil(
      self.clients.matchAll({ type: 'window' }).then(clients => {
        if (clients.length > 0) {
          clients[0].focus();
...
self.addEventListener('push', e => {
  ...
  const isSilentStatus = (tag === 'pomo-status');
  e.waitUntil(
    self.registration.showNotification(title, {
      body,
      icon: './icon-192.png',
      badge: './icon-192.png',
      tag,
      renotify: !isSilentStatus,
      requireInteraction: true,
      vibrate: isSilentStatus ? [] : vibrate,
      silent: isSilentStatus
    }).then(() => {
